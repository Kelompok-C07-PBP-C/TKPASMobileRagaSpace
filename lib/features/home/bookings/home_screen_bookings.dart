          ...
          .map(
            (entry) => ChoiceChip(
              label: Text(
                entry.value,
                style: GoogleFonts.plusJakartaSans(
                  color: filter == entry.key ? Colors.black : Colors.black.withValues(alpha: 0.9),
                  fontWeight:
                      filter == entry.key ? FontWeight.w700 : FontWeight.w600,
                ),
              ),
...
class _BookingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateRange = "${_formatReadableDate(booking.startDate)} \u2022 ${_formatReadableDate(booking.endDate)}";
    return _GlassPanel(
      radius: 30,
